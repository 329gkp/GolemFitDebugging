...
void splatData(const std::string& filename, const std::deque<Event>& exp, const std::deque<Event>& sim){
    ...
    datafile.write(reinterpret_cast<const char *>(&v_cbor[0]), v_cbor.size());
    datafile.close();
}
...
void unsplatData(const std::string& filename, std::deque<Event>& exp, std::deque<Event>& sim){
    ...
    v_cbor.reserve(fileSize);

    // read the data:
    v_cbor.insert(v_cbor.begin(),
               std::istream_iterator<char>(file),
               std::istream_iterator<char>());

    json j_from_cbor = json::from_cbor(v_cbor);

    std::vector<Event> exp_v(j_from_cbor.at(0).get<std::vector<Event>>());
    exp.clear();
    for(auto e : exp_v)
        exp.push_back(e);

    std::vector<Event> sim_v(j_from_cbor.at(1).get<std::vector<Event>>());
    sim.clear();
    for(auto e : sim_v)
        sim.push_back(e);
}

} // closing jason namespace
...
namespace{
	struct energyRecord{
		double energy;
		phys_tools::tableio::RecordID id;
	};
